...
if (!isConfigured) {
  ...
  console.warn("  - VITE_SUPABASE_URL (from https://app.supabase.com/project/_/settings/api)")
  console.warn("  - VITE_SUPABASE_ANON_KEY (from https://app.supabase.com/project/_/settings/api)")
  console.warn("")
  console.warn("The app will run in guest/demo mode without authentication.")
} else {
...
export async function syncAuthToken() {
  if (!supabase) {
    console.log("[v0] Skipping auth token sync - no Supabase client")
    return
  }
...
export function initAuthSync() {
  if (!supabase) {
    console.log("[v0] Skipping auth sync initialization - no Supabase client")
    return
  }
